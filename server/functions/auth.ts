...
export const registerUser = functions.https.onCall(
  async (data: any, context: functions.https.CallableContext) => {
    ...
    if (!emailVal.valid) {
      throw new functions.https.HttpsError('invalid-argument', emailVal.errors.join('; '));
    }

    const passwordVal = validatePassword(password);
    if (!passwordVal.valid) {
      throw new functions.https.HttpsError('invalid-argument', passwordVal.errors.join('; '));
    }

    const usernameVal = validateUsername(username);
    if (!usernameVal.valid) {
      throw new functions.https.HttpsError('invalid-argument', usernameVal.errors.join('; '));
    }

    const displayNameVal = validateDisplayName(displayName);
    if (!displayNameVal.valid) {
      throw new functions.https.HttpsError('invalid-argument', displayNameVal.errors.join('; '));
    }
...
export const updateUserRole = functions.https.onCall(
  async (
    data: { userId: string; newRole: 'member' | 'partner' | 'admin' | 'founder' | 'support' },
...
export const banUser = functions.https.onCall(
  async (
    ...
    if (!data.reason || data.reason.length < 5) {
      throw new functions.https.HttpsError('invalid-argument', 'Reason is required (min 5 chars)');
    }
